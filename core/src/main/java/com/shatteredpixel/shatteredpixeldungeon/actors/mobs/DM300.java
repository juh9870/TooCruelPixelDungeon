...
public class DM300 extends Mob {

	{
		...
		HP = HT = Challenges.STRONGER_BOSSES.enabled() ? 400 : 300;
		EXP = 30;
		defenseSkill = 15;
	...
	private final int MAX_COOLDOWN = Challenges.STRONGER_BOSSES.enabled() ? 7 : 9;
	...
	public int totalPylonsToActivate(){
		return Challenges.STRONGER_BOSSES.enabled() ? 3 : 2;
	}
	...
	protected boolean getCloser(int target) {
		if (super.getCloser(target)){
			...
			if (!supercharged || state != HUNTING || rooted>0 || target == pos() || Dungeon.level.adjacent(pos(), target)) {
				return false;
			}
		...
		public boolean act() {
			for (int i : rockPositions){
				...
				if (ch != null && !(ch instanceof DM300)){
					Buff.prolong( ch, Paralysis.class, Challenges.STRONGER_BOSSES.enabled() ? 5 : 3 );
				}
			}

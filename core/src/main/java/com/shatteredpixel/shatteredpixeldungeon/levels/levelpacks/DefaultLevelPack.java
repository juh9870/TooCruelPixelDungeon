...
public class DefaultLevelPack extends LinearLevelPack<Marker.Custom> {
    ...
    public boolean posNeeded() {
        ...
        int posLeftThisSet = 2 - (Dungeon.LimitedDrops.STRENGTH_POTIONS.getCount() - curLvl.chapterId() * 2);
    ...
    public boolean souNeeded() {
        ...
        if (Challenges.FORBIDDEN_RUNES.enabled()) {
            souLeftThisSet = Math.round(1.5f - (Dungeon.LimitedDrops.UPGRADE_SCROLLS.getCount() - curLvl.chapterId() * 1.5f));
        } else {
            souLeftThisSet = 3 - (Dungeon.LimitedDrops.UPGRADE_SCROLLS.getCount() - curLvl.chapterId() * 3);
        }
    ...
    public boolean asNeeded() {
        //1 AS each floor set
        int asLeftThisSet = 1 - (Dungeon.LimitedDrops.ARCANE_STYLI.getCount() - curLvl.chapterId());

...
public class Badges {
	...
	public static void validateChampion(Modifiers modifiers) {
		if (!modifiers.isChallenged()) return;
		Difficulty diff = modifiers.difficulty();
		Difficulty[] values = Difficulty.values();
		Badge topBadge = null;
		for (int i = diff.ordinal(); i >= 0; i--) {
			Badge badge = values[i].badge;
			if (badge != null) {
				if (topBadge == null) topBadge = badge;
				else {
					if (!global.contains(badge)) {
						global.add(badge);
						saveNeeded = true;
